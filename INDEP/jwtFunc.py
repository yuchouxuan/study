...
def signToken(headDict, paylDict, key, keyLength):
    ...
    if keyLength == 384:
        newContents = base64.urlsafe_b64encode(json.dumps(newHead, separators=(",", ":")).encode()).decode(
        ...
        newSig = base64.urlsafe_b64encode(hmac.new(key.encode(), newContents.encode(), hashlib.sha384).digest()).decode(
            'UTF-8').strip("=")
        badSig = base64.b64encode(hmac.new(key.encode(), newContents.encode(), hashlib.sha384).digest()).decode(
            'UTF-8').strip("=")
    elif keyLength == 512:
        newContents = base64.urlsafe_b64encode(json.dumps(newHead, separators=(",", ":")).encode()).decode(
            'UTF-8').strip("=") + "." + base64.urlsafe_b64encode(
        ...
        newSig = base64.urlsafe_b64encode(hmac.new(key.encode(), newContents.encode(), hashlib.sha512).digest()).decode(
            'UTF-8').strip("=")
        badSig = base64.b64encode(hmac.new(key.encode(), newContents.encode(), hashlib.sha512).digest()).decode(
            'UTF-8').strip("=")
    else:
        newContents = base64.urlsafe_b64encode(json.dumps(newHead, separators=(",", ":")).encode()).decode(
            'UTF-8').strip("=") + "." + base64.urlsafe_b64encode(
        ...
        newSig = base64.urlsafe_b64encode(hmac.new(key.encode(), newContents.encode(), hashlib.sha256).digest()).decode(
            'UTF-8').strip("=")
        badSig = base64.b64encode(hmac.new(key.encode(), newContents.encode(), hashlib.sha256).digest()).decode(
            'UTF-8').strip("=")
        ...
        if claim in ["exp", "nbf", "iat"]:
            ...
            if claim == "exp":
                if int(timestamp.timestamp()) < nowtime:
                    expiredtoken = True
            print("[" + placeholder + "] " + claim + " = " + str(
                paylDict[claim]) + "    ==> TIMESTAMP = " + timestamp.strftime('%Y-%m-%d %H:%M:%S') + " (UTC)")
            timeseen += 1
...
if __name__ == '__main__':
    ...
    if args.spoofjwk:
        if urlinject:
            ...
            try:
                kid = headDict["kid"]
                newSig, badSig, newContents, newjwks, privKeyName, jwksName = jwksGen(headDict, paylDict, jku,
                                                                                      privateKey, kid)
            except:
                kid = ""
                newSig, badSig, newContents, newjwks, privKeyName, jwksName = jwksGen(headDict, paylDict, jku,
