...
class MutableBytecode():
    def __init__(self, code):
        ...
        self.consts = [MutableBytecode(const) if isinstance(const, types.CodeType) else const for const in
                       code.co_consts]
...
def _parseArgs():
    ...
    argParser.add_argument("-r", "--report", action="store_true",
                           help="Report max available payload size carrier supports")
    argParser.add_argument("-s", "--side-by-side", action="store_true",
                           help="Do not overwrite carrier file, install side by side instead.")
    ...
    argParser.add_argument("-e", "--explode", type=int, default=math.inf,
                           help="Explode payload into groups of a limited length if necessary")
...
def _toCodeType(mutableBytecode):
    return types.CodeType(
        ...
        bytes(mutableBytecode.bytes),
        tuple(
            [_toCodeType(const) if isinstance(const, MutableBytecode) else const for const in mutableBytecode.consts]),
...
def main():
    ...
    if payloadLen > maxPayloadSize:
        sys.exit("Carrier can only support a payload of {} bytes, payload of {} bytes received".format(maxPayloadSize,
                                                                                                       payloadLen))
